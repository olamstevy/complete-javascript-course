...
const calcDisplaySummary = function (account) {
  ...
  const interest = account.movements
    .filter(mov => mov > 0)
    .map(dep => dep * account.interestRate)
    .filter((int, i, arr) => {
      // console.log(arr);
      return int >= 1;
    })
...
const calcAverageHumanAge = function (ages) {
  console.log(
    ages
      .map(age => (age > 2 ? 16 + age * 4 : 2 * age)) // calc human age
      .filter(age => age >= 18) // remove adult
      .reduce((acc, curr, i, arr) => acc + curr / arr.length, 0) // calc avg
  );
};
...
console.log(dogs.some(dog => checkOkay(dog)));

// 7.
const okayDogFood = dogs.filter(dog => checkOkay(dog));
console.log(okayDogFood);

// 8.
const dogs2 = [...dogs];
dogs2.sort((a, b) => a.recFood - b.recFood);
console.log(dogs2);
